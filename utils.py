...
from info import AUTH_CHANNEL, LONG_IMDB_DESCRIPTION, MAX_LIST_ELM, SHORTLINK_URL, SHORTLINK_API, LOG_CHANNEL, GRP_LNK, CHNL_LNK, CUSTOM_FILE_CAPTION, IS_VERIFY, VERIFY2_URL, VERIFY2_API, SYD_SHARE, SYD_LINKS 
...
async def is_subscribed(bot, query=None, userid=None):
    try:
        if userid == None and query != None:
            user = await bot.invite_link(AUTH_CHANNEL, query.user_chat_id)
        else:
            user = await bot.invite_link(AUTH_CHANNEL, int(userid))
